...
class BenchmarkRunner {
  ...
  public async run(): Promise<void> {
    ...
    console.log(`\n📈 All ${this.config.userCount} users created. Waiting for benchmark to complete...`);
    ...
    return new Promise((resolve) => {
      const checkCompletion = () => {
        const allUsersComplete = this.users.every((user) => {
          ...
          return userStats.messagesSent >= this.config.messagesPerUser;
        });
  ...
  private printResults(): void {
    ...
    console.log(`📊 Avg messages per user: ${(this.stats.messagesSent / this.stats.connectionsSuccessful).toFixed(2)}`);
    console.log("=".repeat(50));
